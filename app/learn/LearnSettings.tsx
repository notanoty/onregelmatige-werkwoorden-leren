...
export function LearnSettings({ verbs }: LearnSettingsProps) {
  ...
  const [showAnswerAfterCard, setShowAnswerAfterCard] = useState(false);
  ...
  const handleStartTest = () => {
    const params = new URLSearchParams({
      ...
      showAnswerAfterCard: showAnswerAfterCard ? '1' : '0',
      ...
      <main className="flex flex-col gap-8 px-8 max-w-2xl w-full">
        ...
        <div className="flex flex-col gap-4">
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              type="checkbox"
              checked={showAnswerAfterCard}
              onChange={(e) => setShowAnswerAfterCard(e.target.checked)}
              className="w-6 h-6"
              style={{
                accentColor: '#1a1a1a',
              }}
